...
for t in fetch_fifo_data():
    if len(t) != 6:
        print(f"Skipping invalid row (length != 6): {t}")
        continue
    ...
    grouped[ticker].append((date_str, ticker, action, qty, price, platform))
...
for ticker, txns in grouped.items():
    ...
    summary_list.append({
        ...
        "curent_pe": safe_round(info.get('forwardPE')),
...
if not df.empty:
    ...
    try:
        ...
        INSERT INTO he_portfolio_master (
            ticker, Category, quantity, avg_cost, total_cost, current_price,
            unrealized_gain_loss, relized_gain_loss, first_buy_age, avg_age_days,
            platform, industry_pe, curent_pe, price_sales_ratio, price_book_ratio,
            50_day_ema, 100_day_ema, 200_day_ema, sp_500_ya, nashdaq_ya,
            russel_1000_ya, pe_ratio, peg_ratio, roe, net_profit_margin,
            current_ratio, debt_equity, fcf_yield, revenue_growth, earnings_accuracy, 
            position_size
        ) VALUES (
            %(ticker)s, %(Category)s, %(quantity)s, %(avg_cost)s, %(total_cost)s, 
            %(current_price)s, %(unrealized_gain_loss)s, %(relized_gain_loss)s, 
            %(first_buy_age)s, %(avg_age_days)s, %(platform)s, %(industry_pe)s, 
            %(curent_pe)s, %(price_sales_ratio)s, %(price_book_ratio)s, 
            %(50_day_ema)s, %(100_day_ema)s, %(200_day_ema)s, 
            %(sp_500_ya)s, %(nashdaq_ya)s, %(russel_1000_ya)s,
            %(pe_ratio)s, %(peg_ratio)s, %(roe)s, 
            %(net_profit_margin)s, %(current_ratio)s, 
            %(debt_equity)s, %(fcf_yield)s, 
        ...
        ON DUPLICATE KEY UPDATE
            ...
            nashdaq_ya = VALUES(nashdaq_ya),
            russel_1000_ya = VALUES(russel_1000_ya),
        ...
        print(f"MySQL Insertion Error: {err}")
else:
